    ...
    <el-card shadow="never" body-style="padding-top: 5px;">
      <el-tabs v-model="active" @tab-click="handleClick">
        <el-tab-pane v-for="(item,index) in configList" :key="index" :label="item.config_name" :name="'tab_'+index"
          ...
          <el-form :model="form" ref="form" label-width="150px" v-for="(subItem,val) in item.itemList" :key="val">
            <!-- 单行文本框 -->
...
export default {
  ...
  methods: {
      ...
      this.$http.get('/configweb/index').then(res => {
        if (res.data.code === 0) {
          ...
          if (this.configList.length > 0) {
            let itemList = this.configList[0]['itemList'];
            let itemArr = {};
    ...
    handleClick(tab, event) {
      ...
      let itemList = this.configList[tab.index]['itemList'];
      let itemArr = {};
      itemList.forEach(item => {
        itemArr[item.code] = item.value;
