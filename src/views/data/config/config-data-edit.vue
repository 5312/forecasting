...
<template>
  ...
  >
    <el-form :model="form" ref="form" :rules="rules" label-width="96px">
      <el-row :gutter="15">
        <el-col :sm="12">
          <el-form-item label="配置项标题:" prop="title">
            <el-input
              v-model="form.title"
  ...
  methods: {
    ...
    save () {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          this.loading = true
          this.$http[this.isUpdate ? 'put' : 'post'](this.isUpdate ? '/configdata/update' : '/configdata/add',
            Object.assign({}, this.form, {
              ConfigId: this.dictId
            })
          ).then(res => {
            this.loading = false
            if (res.data.code === 0) {
