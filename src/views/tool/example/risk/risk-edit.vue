...
<template>
  <el-dialog
    ...
    <el-form
      ...
      <el-form-item label="评分ID:" prop="score_id">
        <el-input-number
          :min="0"
      ...
      <el-form-item label="造成安全风险id:" prop="riskdata_id">
        <el-input-number
          :min="0"
          v-model="form.riskdata_id"
          placeholder="请输入造成安全风险id"
          controls-position="right"
          class="ele-fluid ele-text-left"/>
      </el-form-item>
  ...
  data() {
    return {
      ...
      rules: {
		...
		scoreId: [
          {required: true, message: '请输入评分ID', trigger: 'blur'}
        ],
  ...
  methods: {
    ...
    save() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          this.loading = true;
          this.$http[this.form.id ? 'put' : 'post'](this.isUpdate ? '/risk/update' : '/risk/add', this.form).then(res => {
            this.loading = false;
            if (res.data.code === 0) {
