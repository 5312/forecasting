...
<template>
  ...
  >
    <el-form ref="form" :model="form" :rules="rules" label-width="82px">
      ...
      <el-form-item label="资源数据库:" prop="item_id" v-if="isShow">
        <el-input-number
          :min="0"
      ...
      <el-form-item label="类别:" prop="itemcate_id" v-if="isShow">
        <el-input-number
          :min="0"
      ...
      <el-form-item label="栏目:" prop="itemcate_cid" v-if="isShow">
        <el-input-number
          :min="0"
      ...
      <el-form-item label="加减分:" prop="score">
        <el-input v-model="form.score" placeholder="请输入加减分" clearable />
      </el-form-item>
  ...
  data () {
    return {
      ...
      rules: {
        ...
        score: [{ required: true, message: "请输入加减分", trigger: "blur" }],
    ...
    initEditor () {
      return {
        where () {
          return {
            itemId: this.itemId,
            itemcate_id: this.itemId2,
            itemcate_cid: this.itemId3,
          }
        },
        height: 300,
        branding: false,
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          ...
          this.$http[this.form.id ? "put" : "post"](
            this.isUpdate ? "/score/update" : "/score/add",
            this.form
          )
            .then((res) => {
              this.loading = false
